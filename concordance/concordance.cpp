...
void help()
{
	...
	cout << "Options:\n"
		...
		<< " remote\n"
		<< "\t--firmware\t\tRead and remote's firmware and write it"
		<< " to\n"
		...
		<< " supported.\n"
		<< "\t--safemode\t\tRead safemode firmware and write it to\n"
		<< "\t\t\t\tsafe.bin.\n"
		<< "\t--connect\t\tDo a connectivity test\n"
		<< "\t--write\t\t\tDefines another option as being in"
		<< " write mode\n\t\t\t\tinstead of read mode\n"
		<< "\t--binary\t\tThe config file is already truncated\n"
		<< "\t\t\t\tto just the binary portions. If in --write mode\n"
		<< "\t\t\t\tthen write binary only portion out to .bin file\n"
		<< "\t--learn\t\t\tLearn IR commands from other remotes\n"
		<< "\t--reset\t\t\tReboot the remote\n"
 ...
 */
void get_version_info(TRemoteInfo &ri, uint8_t *ser, uint32_t &cookie,
			uint32_t &end)
{
	int err = 0;
 ...
 */
void print_version_info(TRemoteInfo &ri, bool &option_verbose,
...
int main(int argc, char *argv[])
{
	...
	while ((tmpint = getopt_long(argc, argv, "bcCfhl:rv", long_options,
					&option_index)) != EOF) {
		switch (tmpint) {
		case 0:
	 ...
	 *
	 *   FIXME: Rather than track all of this and potentially do bad
	 *   	things, we should simply require the user to specify a mode
	 *   	and only allow one mode at a time. We would do this by
	 *   	having the case statement above do simply:
	 *   		mode = MODE_CONFIG_WRITE;
	 *   	or some-such...
	 *
	 *  - Phil
	 */
	if (option_write && option_config && option_firmware) {
		cerr << "Please specify only one thing to write at a time.\n";
		exit(1);
	}

	if (option_write && (option_learn || option_connect || option_reset)) {
 ...
 */
	...
	get_version_info(ri, ser, cookie, end);
	print_version_info(ri, option_verbose, hid_info, ser);
